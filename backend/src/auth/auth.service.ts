...
export class AuthService {
  ...
  async requestOtp(dto: RequestOtpDto): Promise<{ ok: true; devCode?: string }> {
    ...
    const code = await this.otp.generate(dto.phone)
    await this.notifications.sendOtp(dto.phone, code)
    if (!this.isProd) {
      this.logger.debug(`DEV OTP for ${dto.phone}: ${code}`)
      return { ok: true, devCode: code }
    }
    return { ok: true }
  }

  /** Verify the code; existing user → session, new user → reg-token cookie. */
  async verifyOtp(dto: VerifyOtpDto, res: Response): Promise<{ registered: boolean }> {
    const result = await this.otp.verify(dto.phone, dto.code)
    if (!result.ok) {
      const code =
